...
namespace N1MM
{
    public class N1MMLogSync : N1MMLogBase
    {
        ...
        public N1MMLogSync(LogWriteMessageDelegate mylog) : base(mylog)
        {
            ...
            N1MMlogsyncState = N1MMLOGSYNCSTATE.WAIT_HELLO;
        ...
        private enum N1MMLOGSYNCSTATE { WAIT_HELLO, HELLO_RECEIVED, GET_QSO, QSO_IN_SYNC };

        private N1MMLOGSYNCSTATE n1mmlogsyncState = N1MMLOGSYNCSTATE.WAIT_HELLO;
        private N1MMLOGSYNCSTATE N1MMlogsyncState
        {
            get { return n1mmlogsyncState; }
            set
            {
                if (n1mmlogsyncState != value)
                {
                    n1mmlogsyncState = value;
                    switch (n1mmlogsyncState)
                    {
                        case N1MMLOGSYNCSTATE.QSO_IN_SYNC:
                            LogState = LOG_STATE.LOG_IN_SYNC;
                            break;
                        case N1MMLOGSYNCSTATE.WAIT_HELLO:
        ...
        {
            public enum N1MMAvailableFrom { Owner, LoggedElse };
            public N1MMAvailableFrom AvailableFrom;

            public StationLogStat(string StationUniqueID, N1MMAvailableFrom AvailableFrom, List<logSegment> ls)
        ...
        private class StationSyncStatus : N1MMStatus.N1MMStat
        {
        ...
        private List<StationSyncStatus> N1MMStationSyncList = new List<StationSyncStatus>();

        private List<logState> myLogState = new List<logState>();

        private void send_status()
        {
            N1MMMessage Msg = new N1MMMessage(N1MMMESSAGES.STATUS, my_N1MMname, "", "0 12 0 0 0 \"0\" 0 \"1\" 0 \"\""); //FIXME really const or need to adapt
            send(Msg);
        }

        private void send_needqso(string target_N1MM, string StationUniqueID, uint count_from, uint count_to)
        {
            N1MMMessage Msg = new N1MMMessage(N1MMMESSAGES.NEEDQSO, my_N1MMname, target_N1MM, string.Concat(new string[]
            {
        ...
        private bool find_alternative_source(string original_from, string StationUniqueID)
        {
            ...
            foreach (var sl2 in N1MMStationSyncList)
            {
        ...
        private void ti_ti_get_log_Tick(object sender, ElapsedEventArgs e)
        {
            try
            {
                if (intimer)
                    return;
                if (N1MMlogsyncState != N1MMLOGSYNCSTATE.GET_QSO && N1MMlogsyncState != N1MMLOGSYNCSTATE.QSO_IN_SYNC)
                    return;
                ...
                bool N1MMStationsActive = false;

                lock (N1MMStationSyncList)
                {
                    foreach (var sl in N1MMStationSyncList)
                    {
                        foreach (var ls in sl.logstat)
                        {
                            N1MMStationsActive = true;
                            // prefer owner on first iteration
                            if (ls.AvailableFrom == StationLogStat.N1MMAvailableFrom.Owner)
                            {
                                ...
                                else if (cs == SECTION_STATE.SECTION_TIMEOUT)
                                {
                                    ...
                                    {
                                        // remove the station
                                        N1MMStationSyncList.RemoveAll(x => x.from == sl.from);
                ...
                if (N1MMStationsActive && !needQSOs_sent && !timeoutRemoveStation)
                {
                    if (N1MMlogsyncState == N1MMLOGSYNCSTATE.GET_QSO)
                        N1MMlogsyncState = N1MMLOGSYNCSTATE.QSO_IN_SYNC;
                ...
                {
                    if (N1MMlogsyncState == N1MMLOGSYNCSTATE.QSO_IN_SYNC)
                        N1MMlogsyncState = N1MMLOGSYNCSTATE.GET_QSO;
                }
            }
            catch (Exception ex)
            {
        ...
        public override void Get_QSOs(string N1MMname)
        {
            // TODO: start log sync
            my_N1MMname = N1MMname;
            if (N1MMlogsyncState == N1MMLOGSYNCSTATE.HELLO_RECEIVED)
                N1MMlogsyncState = N1MMLOGSYNCSTATE.GET_QSO;
        }
        ...
        {
            Debug("N1MM Msg " + e.Msg.Msg + " src " +
            e.Msg.Src + " dest " + e.Msg.Dst + " data " + e.Msg.Data + " checksum "
            + e.Msg.HasChecksum);
            if (e.Msg.Msg == N1MMMESSAGES.HELLO && e.Msg.HasChecksum)
            {
                /*
                ...
                /*
                 * N1MM Msg HELLO src mm dest  data 11290 130 SLAVE 20 2 1601733638 checksum True
                 * N1MM Msg HELLO src mm2 dest  data 2634 130 SLAVE 20 2 1601733638 checksum True
                 */
                ...
                if (N1MMlogsyncState == N1MMLOGSYNCSTATE.WAIT_HELLO)
                    N1MMlogsyncState = N1MMLOGSYNCSTATE.HELLO_RECEIVED;

                StationSyncStatus w = new StationSyncStatus(e.Msg.Src, first_QSO_ts);
                lock (N1MMStationSyncList)
                {
                    int sl_index = N1MMStationSyncList.FindLastIndex(x => x.from == e.Msg.Src);
                    if (sl_index != -1)
                    {
                        N1MMStationSyncList[sl_index].timestamp = w.timestamp;
                        N1MMStationSyncList[sl_index].first_QSO_ts = w.first_QSO_ts;
                    }
                    else
                    {
                        N1MMStationSyncList.Add(w);
                    }
                }
            }
            else
            ...
            if (e.Msg.Msg == N1MMMESSAGES.STATUS && e.Msg.HasChecksum)
            {
                ...
                if (e.Msg.Msg == N1MMMESSAGES.IHAVE && e.Msg.HasChecksum)
            {
                /*
                ...
                */
                ...
                if (data.Length == 5 && (e.Msg.Dst == "" || e.Msg.Dst == my_N1MMname))
                {
                    ...
                    StationLogStat.N1MMAvailableFrom AvailableFrom;

                    if (data[1] == "E" || data[1] == "LOGGEDELSE")
                        AvailableFrom = StationLogStat.N1MMAvailableFrom.LoggedElse;
                    else
                        AvailableFrom = StationLogStat.N1MMAvailableFrom.Owner;
                    ...
                    lock (N1MMStationSyncList)
                    {
                        ...
                        if (sl_index != -1)
                        {
                            int ssl_index = N1MMStationSyncList[sl_index].logstat.FindLastIndex(x => x.StationUniqueID == stationUniqueID);
                            if (ssl_index != -1)
                            {
                                if (N1MMStationSyncList[sl_index].logstat[ssl_index] != sls)
                                    N1MMStationSyncList[sl_index].logstat[ssl_index] = sls;
                            }
                            else
                            {
                                N1MMStationSyncList[sl_index].logstat.Add(sls);
                            }
            ...
            if (e.Msg.Msg == N1MMMESSAGES.ADDQSO && e.Msg.HasChecksum)
            {
                /*
                    #
                ...
                */
                ...
                try
                {
                    band = Enum.Parse(typeof(N1MMBANDS), data[4]).ToString().Replace("Band", "").Replace("Hz", "");
                }
                catch
                {
                    band = ""; // FIXME
                ...
                if (e.Msg.Dst == "" || e.Msg.Dst == my_N1MMname)
                {
                    try
                    {
            ...
            else if (e.Msg.Msg == N1MMMESSAGES.UPDQSO && e.Msg.HasChecksum)
            {
                /*
