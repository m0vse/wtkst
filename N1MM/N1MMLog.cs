...
namespace N1MM
{
    public class N1MMLog : N1MMLogBase
    {
        private string N1MM_FileName = "";

        public N1MMLog(LogWriteMessageDelegate mylog) : base(mylog)
        {
        ...
        {
            return N1MM_FileName + " " + QSO.Rows.Count;
        }

        public override void Get_QSOs(string N1MM_INI_FileName)
        {
            try
            {
                using (StreamReader sr = new StreamReader(N1MM_INI_FileName, Encoding.Default))
                {
                ...
                using (Stream stream = File.Open(N1MM_FileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                {
                    QSO.Clear();
                    byte[] bufh = new byte[13944];
                    stream.Read(bufh, 0, bufh.Length);
                    string N1MMLoc = Encoding.ASCII.GetString(bufh, 24, 6);
                    char[] separator = new char[1];
                    MyLoc = N1MMLoc.Split(separator)[0];
            ...
            {
                Error("(" + N1MM_INI_FileName + "): " + e.Message);
                throw e;
            }
        }
